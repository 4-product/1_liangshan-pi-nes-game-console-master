...
//字库存放起始地址 
#define FONTINFOADDR 	1024*1024*4 					//Explorer STM32F4是从12M地址以后开始存放字库
														//前面12M被fatfs占用了.
...
//		LCD_ShowString(x+3*size/2,y,240,320,size,"%");		
        LCD_ShowString(x+3*size/2,y,"%",RED,WHITE,16,0);
		t=prog;
		if(t>100)t=100;
//		LCD_ShowNum(x,y,t,3,size);//显示数值
        LCD_ShowIntNum(x,y,t,3,RED,WHITE,16);
	}
	return 0;					    
} 
//更新某一个
...
u8 updata_fontx(u16 x,u16 y,u8 size,u8 *fxpath,u8 fx)
{
 	...
 	if(rval==0)	 
	{
		...
		{
			case 0:												//更新UNIGBK.BIN
				ftinfo.ugbkaddr=FONTINFOADDR+512;	//信息头之后，紧跟UNIGBK转换码表  sizeof(ftinfo)
...
{	
	...
	if(rval==0)//字库文件都存在.
	{  
        LCD_ShowString(30,260,"Erasing sectors...",RED,WHITE,16,0);//提示正在擦除扇区
