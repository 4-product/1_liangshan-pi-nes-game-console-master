...
uint8_t pwm_motor_status = 0; // 震动马达的状态  0 关闭 1开始 
...
void motro_gpio_config(void)
{
	...
	/* 配置GPIO的模式 */
	gpio_mode_set(BSP_PWM_PORT,GPIO_MODE_OUTPUT,GPIO_PUPD_NONE,BSP_PWM_PIN);
	/* 配置GPIO的输出 */
...
{
	timer_channel_output_pulse_value_config(BSP_PWM_TIMER,BSP_PWM_CHANNEL,value); // 配置定时器通道输出脉冲值                                                               // 延时50ms
}
