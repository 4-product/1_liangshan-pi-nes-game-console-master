...
u16	    FLASH_SECTOR_COUNT=2048*4;	//W25Q128,前12M字节给FATFS占用
...
)
{
	...
	{
		case SD_CARD://SD卡
			res=sd_block_read((u32*)buff,sector<<9,count);	 
		...
		case EX_FLASH://外部flash
			for(;count>0;count--)
			{
                ...
                //printf("W25QXX_Read sector: %d  count: %d \r\n",sector,count);
				sector++;
...
)
{
	...
	{
		case SD_CARD://SD卡
			res=sd_block_write((u32*)buff,sector<<9,count);
            if(res == SD_OK) res=0;
			while(res )//写出错
			{
